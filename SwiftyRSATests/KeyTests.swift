...
class PublicKeyTests: XCTestCase {
    ...
    func test_initWithData() throws {
        let path = bundle.path(forResource: "swiftyrsa-public", ofType: "der")!
    ...
    func test_initWithBase64String() throws {
        let path = bundle.path(forResource: "swiftyrsa-public-base64", ofType: "txt")!
...
class PrivateKeyTests: XCTestCase {
    ...
    func test_initWithPEMString() throws {
        let path = bundle.path(forResource: "swiftyrsa-private", ofType: "pem")!
        let str = try String(contentsOf: URL(fileURLWithPath: path), encoding: .utf8)
    ...
    func test_initWithPEMStringHeaderless() throws {
        let path = bundle.path(forResource: "swiftyrsa-private-headerless", ofType: "pem")!
        let str = try String(contentsOf: URL(fileURLWithPath: path), encoding: .utf8)
        let privateKey = try? PrivateKey(pemEncoded: str)
        XCTAssertNotNil(privateKey)
    }
